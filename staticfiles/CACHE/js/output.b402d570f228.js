...
addButton.addEventListener('click',addForm)
function addForm(e){e.preventDefault()
let formTemplate=document.querySelector(".image-form").cloneNode(true)
...
field.id=newId
field.name=newName})
formTemplate.querySelectorAll('label').forEach(function(label){let newFor=label.getAttribute('for').replace(/-\d+-/,`-${formNum}-`)
label.setAttribute('for',newFor)
label.innerHTML=label.innerHTML.replace(/#\d+/g,`#${formNum}`)})
